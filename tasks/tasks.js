...
exports.getInstructions = exports.getTaskCategory = exports.tasksValues = exports.tasksPrompt = void 0;
const tasksPrompt = (query) => `You are a senior frontend engineer who needs to develop a web application using only HTML and Javascript.
...
Brief: ${query}
Spec: {`;
exports.tasksPrompt = tasksPrompt;
const tasksValues = (tasks) => Object.entries(tasks).map(([type, value]) => value.toLowerCase());
exports.tasksValues = tasksValues;
const getTaskCategory = (task) => task.match(/(?:summary)/i)
    ? 'summary'
    : task.match(/(?:layout|template)/i)
        ? 'layout'
        : task.match(/(?:js modules|javascript|interactions?|code|interactivity|events?|application|logic|animations?)/i)
            ? 'script'
            : task.match(/(?:images?|photos?|pictures?|illustrations?|screenshots?|snapshots?)/i)
                ? 'image'
...
const getInstructions = (tasks) => Object.entries(tasks).reduce((acc, [type, value]) => {
    const category = (0, exports.getTaskCategory)(type);
...
const tasks = `Brief: An encyclopedia article explaining what are asteroids, their orbits, their composition, and their dangers. The layout is simple, yet elegant, with a blue background, green titles, and a few pictures of asteroids and diagrams. The text is clear and scientific.
Spec: {
  "layout": "An encyclopedia-like layout, similar to scientific articles or Wikipedia. Images (if any) should be including within the body.",
  "art direction": "The design should be sober and elegant, without any emoji, using a serif font to make it looks like a book or printed article. Background should be white, foreground colors should be in shades of grey.",
  "text content": "The article should be written in a very rich and scientific tone, with a lot of details about the origins of the word asteroid, the history of their discovery, a scientific analysis of common asteroid composition, the impact on human society, the economics of asteroids, the risks associated with asteroids, and so on.",
...
console.log((0, exports.getInstructions)(Tasks));
